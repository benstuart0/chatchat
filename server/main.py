...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, username: str) -> str:
        ...
        logger.info(
            "New connection: %(username)s (ID: %(id)s) (Total users: %(total)d)",
            {"username": username, "id": user_id, "total": len(self.active_connections)}
        )
    ...
    def disconnect(self, user_id: str):
        if user_id in self.active_connections:
            ...
            logger.info(
                "Disconnected: %(username)s (ID: %(id)s) (Total users: %(total)d)",
                {"username": username, "id": user_id, "total": len(self.active_connections)}
            )
            return username
    ...
    async def broadcast_user_list(self):
        ...
        await self.broadcast({"type": "users", "content": active_users})
...
@app.get("/")
async def read_root():
    return {"status": "Server is running", "allowed_origins": ALLOWED_ORIGINS}
